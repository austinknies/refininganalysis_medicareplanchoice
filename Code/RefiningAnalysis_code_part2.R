...
# Creates Figure 3, Tables 1-4, Tables 9-12
...
iu2015 <- import("Cost Supplement 2015/Data/CSV Files/iue.csv") %>%
  as_tibble() %>%
  dplyr::select(BASEID,SURVEYYR,AMTTOT,AMTCOV,AMTNCOV, AMTCARE, AMTCAID, AMTMADV, AMTHMOP, AMTPRVE, AMTPRVI, AMTPRVU, AMTOOP, AMTDISC, AMTVA, AMTOTH,SOWMP) %>%
  mutate(AMTOTH = AMTVA + AMTOTH, # After 2015, CMS includes AMTVA in AMTOTH amount; only do for 2015 files
         SOURCETYPE = "IU") %>%
  dplyr::select(-AMTVA)
...
iu2016 <- import("Cost Supplement 2016/Data/CSV Files/iue.csv") %>%
  as_tibble() %>%
  dplyr::select(BASEID,SURVEYYR,AMTTOT,AMTCOV,AMTNCOV, AMTCARE, AMTCAID, AMTMADV, AMTHMOP, AMTPRVE, AMTPRVI, AMTPRVU, AMTOOP, AMTDISC, AMTOTH,SOWMP) %>%
...
mp2015 <- import("Cost Supplement 2015/Data/CSV Files/mpe.csv") %>%
  as_tibble() %>%
  dplyr::select(BASEID,SURVEYYR,AMTTOT,AMTCOV,AMTNCOV, AMTCARE, AMTCAID, AMTMADV, AMTHMOP, AMTPRVE, AMTPRVI, AMTPRVU, AMTOOP, AMTDISC, AMTVA, AMTOTH, PROVSPEC, OMETYPE, OREVTYPE, SOWMP) %>%
  mutate(AMTOTH = AMTVA + AMTOTH, # After 2015, CMS includes AMTVA in AMTOTH amount; only do for 2015 files
         SOURCETYPE = "MP") %>%
  dplyr::select(-AMTVA)
...
mp2016 <- import("Cost Supplement 2016/Data/CSV Files/mpe.csv") %>%
  as_tibble() %>%
  dplyr::select(BASEID,SURVEYYR,AMTTOT,AMTCOV,AMTNCOV, AMTCARE, AMTCAID, AMTMADV, AMTHMOP, AMTPRVE, AMTPRVI, AMTPRVU, AMTOOP, AMTDISC, AMTOTH, PROVSPEC, OMETYPE, OREVTYPE, SOWMP) %>%
...
# 3) Get supplemental coverage and Part D coverage ----
...
## Enrollment in Medicare by Coverage Type, 2015-2019 (Figure 3) ####

groupdf <- mainps %>% 
  group_by(SURVEYYR,GROUPING) %>% 
  summarize(BENNO = sum(CSEVRWGT))

groupdf <- groupdf %>% 
  group_by(SURVEYYR) %>% mutate(TOTAL = sum(BENNO)) %>% ungroup() %>% 
  mutate(SHARE = BENNO/TOTAL)


ggplot()+
  geom_line(aes(x=SURVEYYR, y=SHARE, color=GROUPING), linewidth = 2, data=groupdf)+
  scale_y_continuous(labels = scales::percent, limits = c(0,0.7), expand = c(0,0))+
  scale_x_continuous(expand=c(0,0))+
  labs(x = 'Survey Year', y = 'Share of Enrolled Beneficiaries', color = 'Coverage')+
  theme_bw() 
ggsave("Output/mcrbyyeargrouping.pdf",width = 6, height = 4)

groupdf <- mainps %>% 
  group_by(SURVEYYR,TM) %>% 
  summarize(BENNO = sum(CSEVRWGT))
...
ggplot()+
  geom_line(aes(x=SURVEYYR, y=SHARE, color=as.factor(TM)), linewidth = 2, data=groupdf)+
  scale_y_continuous(labels = scales::percent, limits = c(0,0.7), expand = c(0,0))+
  scale_x_continuous(expand=c(0,0))+
  labs(x = 'Survey Year', y = 'Share of Enrolled Beneficiaries', color = 'Coverage')+
  scale_color_discrete(labels=c("MA","TM"))+
  theme_bw() 
ggsave("Output/mcrbyyeartm.pdf",width = 6, height = 4)

## Summary Statistics Tables #####

mainpssummary <- mainps %>% 
  drop_na(TM,TMONLY,TMPLUS,MAPLUS,TMMEDICAID,MAMEDICAID,PARTD,FULLRISK,H_AGE_DMND,RACMIN,LOGINCOME,HSEDUC,FEMALE,CBSA_D,SURVEYYR)

### Demographic Summary by Year (Table 1) #####

c1<-table1(~GROUPING+PARTD+FULLRISK+H_AGE+RACMIN+INCOME_H+HSEDUC+FEMALE+CBSA_D|as.factor(SURVEYYR), data = mainpssummary)
t1kable(c1, format = "latex") %>% save_kable("Output/summarystatistics_mainpaper.tex")
rm(c1)
gc()

### Demographic Summary by Year (Table 11) #####

c1<-table1(~GROUPING+as.factor(PARTD)+FULLRISK+H_AGE+as.factor(H_RTIRCE)+INCOME_H+as.factor(HSEDUC)+as.factor(H_SEX)+CBSA_D|as.factor(SURVEYYR), data = mainpssummary)
t1kable(c1, format = "latex") %>% save_kable("Output/summarystatistics_byyearappendix.tex")
rm(c1)
gc()

### Demographic Summary by Coverage (Table 12) #####

mainpssummary <- mainpssummary %>% 
  mutate(NUMGROUPING = case_when(GROUPING=="MA Only"~1,
                                 GROUPING=="TM Only"~2,
                                 GROUPING=="TM + Sup Cov"~3,
                                 GROUPING=="MA + Sup Cov"~4,
                                 GROUPING=="TM + Medicaid"~5,
                                 GROUPING=="MA + Medicaid"~6))

c1<-table1(~as.factor(SURVEYYR)+as.factor(PARTD)+FULLRISK+H_AGE+as.factor(H_RTIRCE)+INCOME_H+as.factor(HSEDUC)+as.factor(H_SEX)+CBSA_D|TM*NUMGROUPING, data = mainpssummary, overall = F)
t1kable(c1, format = "latex") %>% save_kable("Output/summarystatistics_bycovappendix.tex")
rm(c1)
gc()

## Impact of Medicare Coverage on Annual Medical Events (Table 2) #####

mainall <- mainall %>% 
  mutate(ERVISIT = case_when(OREVTYPE=="ER"~1,
                             is.na(OREVTYPE)==TRUE~0,
                             TRUE~0))
pseronly <- mainall %>% 
  filter(ERVISIT==1) %>% 
  filter(SOWMP!=1) %>% # KEY FOR ADJUSTING TO CORRECT MCBS SPECIFICATION
  group_by(BASEID,SURVEYYR) %>% 
  summarize(EREVENTS = n())

mainps <- left_join(mainps,pseronly, by = c("BASEID","SURVEYYR"))

mainps <- mainps %>% 
  mutate(EREVENTS = case_when(is.na(EREVENTS)==TRUE~0,
                              TRUE~EREVENTS)) %>% 
  mutate(NONPMER = PEVENTS - PMAEVNTS - EREVENTS)

reg.1 <- glm(PEVENTS ~ 1+TM+PARTD+FULLRISK+H_AGE_DMND+RACMIN+LOGINCOME+as.factor(HSEDUC)+FEMALE+as.factor(CBSA_D)+as.factor(SURVEYYR), family = "poisson", data = mainps, weights = CSEVRWGT)
reg.2 <- glm(PMAEVNTS ~ 1+TM+PARTD+FULLRISK+H_AGE_DMND+RACMIN+LOGINCOME+as.factor(HSEDUC)+FEMALE+as.factor(CBSA_D)+as.factor(SURVEYYR), family = "poisson", data = mainps, weights = CSEVRWGT)
reg.3 <- glm(NONPMER ~ 1+TM+PARTD+FULLRISK+H_AGE_DMND+RACMIN+LOGINCOME+as.factor(HSEDUC)+FEMALE+as.factor(CBSA_D)+as.factor(SURVEYYR), family = "poisson", data = mainps, weights = CSEVRWGT)
reg.4 <- glm(EREVENTS ~ 1+TM+PARTD+FULLRISK+H_AGE_DMND+RACMIN+LOGINCOME+as.factor(HSEDUC)+FEMALE+as.factor(CBSA_D)+as.factor(SURVEYYR), family = "poisson", data = mainps, weights = CSEVRWGT)
reg.5 <- glm(PEVENTS ~ 1+TMONLY+TMPLUS+MAPLUS+TMMEDICAID+MAMEDICAID+PARTD+FULLRISK+H_AGE_DMND+RACMIN+LOGINCOME+as.factor(HSEDUC)+FEMALE+as.factor(CBSA_D)+as.factor(SURVEYYR), family = "poisson", data = mainps, weights = CSEVRWGT)
reg.6 <- glm(PMAEVNTS ~ 1+TMONLY+TMPLUS+MAPLUS+TMMEDICAID+MAMEDICAID+PARTD+FULLRISK+H_AGE_DMND+RACMIN+LOGINCOME+as.factor(HSEDUC)+FEMALE+as.factor(CBSA_D)+as.factor(SURVEYYR), family = "poisson", data = mainps, weights = CSEVRWGT)
reg.7 <- glm(NONPMER ~ 1+TMONLY+TMPLUS+MAPLUS+TMMEDICAID+MAMEDICAID+PARTD+FULLRISK+H_AGE_DMND+RACMIN+LOGINCOME+as.factor(HSEDUC)+FEMALE+as.factor(CBSA_D)+as.factor(SURVEYYR), family = "poisson", data = mainps, weights = CSEVRWGT)
reg.8 <- glm(EREVENTS ~ 1+TMONLY+TMPLUS+MAPLUS+TMMEDICAID+MAMEDICAID+PARTD+FULLRISK+H_AGE_DMND+RACMIN+LOGINCOME+as.factor(HSEDUC)+FEMALE+as.factor(CBSA_D)+as.factor(SURVEYYR), family = "poisson", data = mainps, weights = CSEVRWGT)


model.lst = list(reg.1,reg.2,reg.3,reg.4,reg.5,reg.6,reg.7,reg.8)
...
stargazer(reg.1,reg.2,reg.3,reg.4,reg.5,reg.6,reg.7,reg.8,
          ...
          out = "Output/totalevents_risk.tex")



## Impact of Medicare Coverage on Total and OOP Expenses (Non-PM and Non-ER Events) (Table 3) #####

reg.1 <- glm(AMTTOT ~ 1+TM+PARTD+FULLRISK+H_AGE_DMND+RACMIN+LOGINCOME+as.factor(HSEDUC)+FEMALE+as.factor(CBSA_D)+as.factor(SURVEYYR)+as.factor(SOURCETYPE), family = "poisson", data = subset(mainall, SOURCETYPE!="PM" & ERVISIT==0), weights = CSEVRWGT)
reg.2 <- glm(AMTOOP ~ 1+TM+PARTD+FULLRISK+H_AGE_DMND+RACMIN+LOGINCOME+as.factor(HSEDUC)+FEMALE+as.factor(CBSA_D)+as.factor(SURVEYYR)+as.factor(SOURCETYPE), family = "poisson", data = subset(mainall, SOURCETYPE!="PM" & ERVISIT==0), weights = CSEVRWGT)
reg.3 <- glm(AMTTOT ~ 1+TMONLY+TMPLUS+MAPLUS+TMMEDICAID+MAMEDICAID+PARTD+FULLRISK+H_AGE_DMND+RACMIN+LOGINCOME+as.factor(HSEDUC)+FEMALE+as.factor(CBSA_D)+as.factor(SURVEYYR)+as.factor(SOURCETYPE), family = "poisson", data = subset(mainall, SOURCETYPE!="PM" & ERVISIT==0), weights = CSEVRWGT)
reg.4 <- glm(AMTOOP ~ 1+TMONLY+TMPLUS+MAPLUS+TMMEDICAID+MAMEDICAID+PARTD+FULLRISK+H_AGE_DMND+RACMIN+LOGINCOME+as.factor(HSEDUC)+FEMALE+as.factor(CBSA_D)+as.factor(SURVEYYR)+as.factor(SOURCETYPE), family = "poisson", data = subset(mainall, SOURCETYPE!="PM" & ERVISIT==0), weights = CSEVRWGT)
...
stargazer(reg.1,reg.2,reg.3,reg.4, 
          ...
          font.size = "small",
          notes = "robust SEs in parentheses",
          out = "Output/perevent_risk_nonpmer.tex")


## Impact of Medicare Coverage on Annual Cost Differences (Table 4) #####

pmdf <- mainall %>% 
  filter(ERVISIT==0 & SOURCETYPE=="PM") %>% 
  filter(SOWMP!=1|is.na(SOWMP)==TRUE) %>%  
  group_by(BASEID,SURVEYYR) %>% 
  summarize(SUBAMTTOT = sum(AMTTOT),
            SUBAMTOOP = sum(AMTOOP)) %>% 
  mutate(PMADIFF = SUBAMTTOT - SUBAMTOOP) %>% 
  dplyr::select(BASEID, SURVEYYR, PMADIFF)
mainpstable4 <- left_join(mainps,pmdf, by = c("BASEID","SURVEYYR"))
rm(pmdf)
...
nonpmerdf <- mainall %>% 
  ...
  summarize(SUBAMTTOT = sum(AMTTOT),
            SUBAMTOOP = sum(AMTOOP)) %>% 
  mutate(NONPMERDIFF = SUBAMTTOT - SUBAMTOOP) %>% 
  dplyr::select(BASEID, SURVEYYR, NONPMERDIFF)
mainpstable4 <- left_join(mainpstable4,nonpmerdf, by = c("BASEID","SURVEYYR"))
rm(nonpmerdf)

erdf <- mainall %>% 
  filter(ERVISIT==1 & SOURCETYPE!="PM") %>% 
  filter(SOWMP!=1|is.na(SOWMP)==TRUE) %>%  
  group_by(BASEID,SURVEYYR) %>% 
  summarize(SUBAMTTOT = sum(AMTTOT),
            SUBAMTOOP = sum(AMTOOP)) %>% 
  mutate(ERDIFF = SUBAMTTOT - SUBAMTOOP) %>% 
  dplyr::select(BASEID, SURVEYYR, ERDIFF)
mainpstable4 <- left_join(mainpstable4,erdf, by = c("BASEID","SURVEYYR"))
rm(erdf)

mainpstable4 <- mainpstable4 %>% 
  mutate(PDIFF = PAMTTOT - PAMTOOP)

reg.1 <- lm(PDIFF ~ 1+TM+PARTD+FULLRISK+H_AGE_DMND+RACMIN+LOGINCOME+as.factor(HSEDUC)+FEMALE+as.factor(CBSA_D)+as.factor(SURVEYYR), data = mainpstable4, weights = CSEVRWGT)
reg.2 <- lm(PMADIFF ~ 1+TM+PARTD+FULLRISK+H_AGE_DMND+RACMIN+LOGINCOME+as.factor(HSEDUC)+FEMALE+as.factor(CBSA_D)+as.factor(SURVEYYR), data = mainpstable4, weights = CSEVRWGT)
reg.3 <- lm(NONPMERDIFF ~ 1+TM+PARTD+FULLRISK+H_AGE_DMND+RACMIN+LOGINCOME+as.factor(HSEDUC)+FEMALE+as.factor(CBSA_D)+as.factor(SURVEYYR), data = mainpstable4, weights = CSEVRWGT)
reg.4 <- lm(ERDIFF ~ 1+TM+PARTD+FULLRISK+H_AGE_DMND+RACMIN+LOGINCOME+as.factor(HSEDUC)+FEMALE+as.factor(CBSA_D)+as.factor(SURVEYYR), data = mainpstable4, weights = CSEVRWGT)
reg.5 <- lm(PDIFF ~ 1+TMONLY+TMPLUS+MAPLUS+TMMEDICAID+MAMEDICAID+PARTD+FULLRISK+H_AGE_DMND+RACMIN+LOGINCOME+as.factor(HSEDUC)+FEMALE+as.factor(CBSA_D)+as.factor(SURVEYYR), data = mainpstable4, weights = CSEVRWGT)
reg.6 <- lm(PMADIFF ~ 1+TMONLY+TMPLUS+MAPLUS+TMMEDICAID+MAMEDICAID+PARTD+FULLRISK+H_AGE_DMND+RACMIN+LOGINCOME+as.factor(HSEDUC)+FEMALE+as.factor(CBSA_D)+as.factor(SURVEYYR), data = mainpstable4, weights = CSEVRWGT)
reg.7 <- lm(NONPMERDIFF ~ 1+TMONLY+TMPLUS+MAPLUS+TMMEDICAID+MAMEDICAID+PARTD+FULLRISK+H_AGE_DMND+RACMIN+LOGINCOME+as.factor(HSEDUC)+FEMALE+as.factor(CBSA_D)+as.factor(SURVEYYR), data = mainpstable4, weights = CSEVRWGT)
reg.8 <- lm(ERDIFF ~ 1+TMONLY+TMPLUS+MAPLUS+TMMEDICAID+MAMEDICAID+PARTD+FULLRISK+H_AGE_DMND+RACMIN+LOGINCOME+as.factor(HSEDUC)+FEMALE+as.factor(CBSA_D)+as.factor(SURVEYYR), data = mainpstable4, weights = CSEVRWGT)
...
stargazer(reg.1,reg.2,reg.3,reg.4,reg.5,reg.6,reg.7,reg.8,
          ...
          font.size = "small",
          notes = "robust SEs in parentheses",
          out = "Output/diffcost_risk.tex")

## Impact of Medicare Coverage on Total and OOP Expenses (Non-PM and Non-ER Events; Excluding Dental, Vision, and Hearing Events) (Table 9) #####

mainall <- mainall %>% 
  mutate(MPDHV = case_when(PROVSPEC %in% c(1,3,7,16,35,37)~1,
                           OMETYPE %in% c(1,2)~1,
                           TRUE~0))

reg.1 <- glm(AMTTOT ~ 1+TM+PARTD+FULLRISK+H_AGE_DMND+RACMIN+LOGINCOME+as.factor(HSEDUC)+FEMALE+as.factor(CBSA_D)+as.factor(SURVEYYR)+as.factor(SOURCETYPE), family = "poisson", data = subset(mainall, SOURCETYPE!="PM" & ERVISIT==0 & SOURCETYPE!="DU" & SOURCETYPE!="HU" & SOURCETYPE!="VU" & MPDHV==0), weights = CSEVRWGT)
reg.2 <- glm(AMTOOP ~ 1+TM+PARTD+FULLRISK+H_AGE_DMND+RACMIN+LOGINCOME+as.factor(HSEDUC)+FEMALE+as.factor(CBSA_D)+as.factor(SURVEYYR)+as.factor(SOURCETYPE), family = "poisson", data = subset(mainall, SOURCETYPE!="PM" & ERVISIT==0 & SOURCETYPE!="DU" & SOURCETYPE!="HU" & SOURCETYPE!="VU" & MPDHV==0), weights = CSEVRWGT)
reg.3 <- glm(AMTTOT ~ 1+TMONLY+TMPLUS+MAPLUS+TMMEDICAID+MAMEDICAID+PARTD+FULLRISK+H_AGE_DMND+RACMIN+LOGINCOME+as.factor(HSEDUC)+FEMALE+as.factor(CBSA_D)+as.factor(SURVEYYR)+as.factor(SOURCETYPE), family = "poisson", data = subset(mainall, SOURCETYPE!="PM" & ERVISIT==0 & SOURCETYPE!="DU" & SOURCETYPE!="HU" & SOURCETYPE!="VU" & MPDHV==0), weights = CSEVRWGT)
reg.4 <- glm(AMTOOP ~ 1+TMONLY+TMPLUS+MAPLUS+TMMEDICAID+MAMEDICAID+PARTD+FULLRISK+H_AGE_DMND+RACMIN+LOGINCOME+as.factor(HSEDUC)+FEMALE+as.factor(CBSA_D)+as.factor(SURVEYYR)+as.factor(SOURCETYPE), family = "poisson", data = subset(mainall, SOURCETYPE!="PM" & ERVISIT==0 & SOURCETYPE!="DU" & SOURCETYPE!="HU" & SOURCETYPE!="VU" & MPDHV==0), weights = CSEVRWGT)
...
stargazer(reg.1,reg.2,reg.3,reg.4, 
          ...
          font.size = "small",
          notes = "robust SEs in parentheses",
          out = "Output/perevent_risk_nonpmer_nodvhatall.tex")


## Impact of Medicare Coverage on Annual Medical Events (Excluding Dental, Vision, and Hearing Events) (Table 10) #####

subdf <- mainall %>% 
  filter(ERVISIT==0 & SOURCETYPE!="PM") %>% 
  filter(SOWMP!=1|is.na(SOWMP)==TRUE) %>% 
  filter(SOURCETYPE=="DU"|SOURCETYPE=="HU"|SOURCETYPE=="VU"|MPDHV==1) %>% 
  group_by(BASEID,SURVEYYR) %>% 
  summarize(SUBEVENTS = n())

mainpsadj <- left_join(mainps,subdf, by = c("BASEID","SURVEYYR"))

mainpsadj <- mainpsadj %>% 
  mutate(PEVENTS = case_when(is.na(SUBEVENTS)==FALSE & SUBEVENTS<=PEVENTS~PEVENTS - SUBEVENTS,
                             is.na(SUBEVENTS)==TRUE~PEVENTS,
                             is.na(SUBEVENTS)==FALSE & SUBEVENTS>PEVENTS~0),
         NONPMER = case_when(is.na(SUBEVENTS)==FALSE & SUBEVENTS<=NONPMER~NONPMER - SUBEVENTS,
                             is.na(SUBEVENTS)==TRUE~NONPMER,
                             is.na(SUBEVENTS)==FALSE & SUBEVENTS>NONPMER~0))

reg.1 <- glm(PEVENTS ~ 1+TM+PARTD+FULLRISK+H_AGE_DMND+RACMIN+LOGINCOME+as.factor(HSEDUC)+FEMALE+as.factor(CBSA_D)+as.factor(SURVEYYR), family = "poisson", data = mainpsadj, weights = CSEVRWGT)
reg.2 <- glm(NONPMER ~ 1+TM+PARTD+FULLRISK+H_AGE_DMND+RACMIN+LOGINCOME+as.factor(HSEDUC)+FEMALE+as.factor(CBSA_D)+as.factor(SURVEYYR), family = "poisson", data = mainpsadj, weights = CSEVRWGT)
reg.3 <- glm(PEVENTS ~ 1+TMONLY+TMPLUS+MAPLUS+TMMEDICAID+MAMEDICAID+PARTD+FULLRISK+H_AGE_DMND+RACMIN+LOGINCOME+as.factor(HSEDUC)+FEMALE+as.factor(CBSA_D)+as.factor(SURVEYYR), family = "poisson", data = mainpsadj, weights = CSEVRWGT)
reg.4 <- glm(NONPMER ~ 1+TMONLY+TMPLUS+MAPLUS+TMMEDICAID+MAMEDICAID+PARTD+FULLRISK+H_AGE_DMND+RACMIN+LOGINCOME+as.factor(HSEDUC)+FEMALE+as.factor(CBSA_D)+as.factor(SURVEYYR), family = "poisson", data = mainpsadj, weights = CSEVRWGT)
...
stargazer(reg.1,reg.2,reg.3,reg.4,
          ...
          font.size = "small",
          notes = "robust SEs in parentheses",
          out = "Output/totalevents_risk_nodhvatall.tex")
